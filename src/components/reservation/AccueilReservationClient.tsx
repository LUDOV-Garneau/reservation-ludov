...
export default function AccueilReservationsClient({ username }: AccueilReservationsClientProps) {
  ...
  useEffect(() => {
    async function fetchReservations() {
      try {
        ...
        const upcomingReservationsResponse = await fetch(
          "/api/reservation/upcoming-reservations"
        );
        ...
        const pastReservationsResponse = await fetch(
          "/api/reservation/past-reservations"
        );
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 sm:gap-0">
          ...
          <Link href="/reservation">
            <Button className="bg-white hover:bg-green-600 text-green-500 border border-green-500 hover:text-white">
              {t("reservation.new")}
            </Button>
          </Link>
        </div>
        ...
        {upcomingReservations.length > 0 ? (
          ...
          <div className="w-full rounded-lg shadow-sm p-6 mb-6 text-center bg-[white]">
            ...
            <p className="text-gray-500">
              {t("reservation.accueil.noUpcomingReservations")}
            </p>
          </div>
        )}
        ...
        {pastReservations.length > 0 ? (
          ...
          <div className="w-full rounded-lg shadow-sm p-6 mb-6 text-center bg-[white]">
            ...
            <p className="text-gray-500">
              {t("reservation.accueil.noPastReservations")}
            </p>
          </div>
        )}
      </div>
