...
import SelectedGameCard from "@/components/reservation/components/SelectedGamesCard";
...
export default function GamesSelection() {
  ...
  const clearGame = (id: number) =>
    setSelected(selected.filter((g) => g.id !== id));

  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="md:col-span-1 space-y-4 sticky top-4 bg-[white] rounded-2xl p-6 m-6 self-start">
        <h2 className="text-xl font-bold mb-4">
          Jeux sélectionnés ({selected.length}/3)
        </h2>
        {selected.length === 0 ? (
      ...
      <div className="md:col-span-2 bg-[white] rounded-2xl p-6 m-6">
        <h2 className="text-xl font-bold mb-4">Sélection de jeux</h2>
        <GameSelectionGrid
