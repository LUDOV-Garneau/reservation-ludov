...
export default function UpdateStationForm({ station, onSuccess, onAlert }: Props) {
    ...
    useEffect(() => {
        ...
        const fetchConsoleStock = async () => {
            try {
                ...
                const initial = data.filter(c => station.consoles.includes(c.id));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const res = await fetch(`/api/admin/update-station/${station.id}`, {
                method: "PUT",
