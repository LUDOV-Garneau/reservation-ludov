/**
 * Formes et rendu des gabarits de courriels — **sans aucun accès à la base**.
 *
 * Séparé de `emailTemplates.ts` parce que ce dernier importe `@/db` : une
 * simple constante importée depuis un composant client embarquait `mysql2`
 * dans le bundle du navigateur, où `net` et `tls` n'existent pas, et le build
 * échouait. Même raison que la note en tête de `dates.ts`.
 *
 * Tout ce qui vit ici est utilisable des deux côtés.
 */
