"use client";

import { useCallback, useMemo } from "react";
import { usePathname, useRouter, useSearchParams } from "next/navigation";

/**
 * État de la vue « Images des jeux » (admin), stocké dans le query string pour
 * survivre à un rechargement, à un aller-retour vers un autre onglet et au
 * partage d'un lien.
 *
 * Deux règles tiennent tout le reste :
 * - une valeur par défaut n'est jamais écrite dans l'URL (elle reste courte, et
 *   une URL nue équivaut à l'état par défaut) ;
 * - une valeur invalide retombe silencieusement sur le défaut, pour qu'une URL
 *   trafiquée à la main ne casse jamais la page.
 */

export const PAGE_SIZE_OPTIONS = [12, 24, 48, 96] as const;
export const DEFAULT_PAGE_SIZE = 12;

export type HasImageFilter = "all" | "yes" | "no";
export type GamesView = "grid" | "table";

export type GamesImagesFiltersState = {
  search: string;
  hasImage: HasImageFilter;
  consoleTypeId: number | null;
  stationId: number | null;
  pageSize: number;
  page: number;
  view: GamesView;
};

/** Clés dont la modification renvoie à la première page. */
const PAGE_RESETTING_KEYS = [
  "search",
  "hasImage",
  "consoleTypeId",
  "stationId",
  "pageSize",
] as const satisfies readonly (keyof GamesImagesFiltersState)[];

const HAS_IMAGE_VALUES: readonly HasImageFilter[] = ["all", "yes", "no"];
const VIEW_VALUES: readonly GamesView[] = ["grid", "table"];

function parsePositiveInt(raw: string | null): number | null {
  if (raw === null || raw.trim() === "") return null;
  const value = Number(raw);
  return Number.isInteger(value) && value > 0 ? value : null;
}

export function useGamesImagesFilters() {
  const searchParams = useSearchParams();
  const pathname = usePathname();
  const router = useRouter();

  const filters = useMemo<GamesImagesFiltersState>(() => {
    const hasImage = searchParams.get("img") as HasImageFilter | null;
    const view = searchParams.get("view") as GamesView | null;
    const size = parsePositiveInt(searchParams.get("size"));

    return {
      search: searchParams.get("q") ?? "",
      hasImage: hasImage && HAS_IMAGE_VALUES.includes(hasImage) ? hasImage : "all",
      consoleTypeId: parsePositiveInt(searchParams.get("console")),
      stationId: parsePositiveInt(searchParams.get("station")),
      pageSize:
        size !== null && (PAGE_SIZE_OPTIONS as readonly number[]).includes(size)
          ? size
          : DEFAULT_PAGE_SIZE,
      page: parsePositiveInt(searchParams.get("page")) ?? 1,
      view: view && VIEW_VALUES.includes(view) ? view : "grid",
    };
  }, [searchParams]);

  const setFilters = useCallback(
    (patch: Partial<GamesImagesFiltersState>) => {
      const resetsPage = PAGE_RESETTING_KEYS.some((key) => key in patch);
      const next: GamesImagesFiltersState = {
        ...filters,
        ...patch,
        page: patch.page ?? (resetsPage ? 1 : filters.page),
      };

      // Les params étrangers au hook sont préservés, à commencer par `tab`.
      const params = new URLSearchParams(searchParams.toString());
      const write = (key: string, value: string, isDefault: boolean) => {
        if (isDefault) params.delete(key);
        else params.set(key, value);
      };

      write("q", next.search, next.search.trim() === "");
      write("img", next.hasImage, next.hasImage === "all");
      write("console", String(next.consoleTypeId), next.consoleTypeId === null);
      write("station", String(next.stationId), next.stationId === null);
      write("size", String(next.pageSize), next.pageSize === DEFAULT_PAGE_SIZE);
      write("page", String(next.page), next.page === 1);
      write("view", next.view, next.view === "grid");

      const query = params.toString();
      // `replace` et non `push` : la recherche est débouncée mais reste
      // frappe-à-frappe, et inonderait l'historique du navigateur.
      router.replace(query ? `${pathname}?${query}` : pathname, {
        scroll: false,
      });
    },
    [filters, pathname, router, searchParams],
  );

  const clearFilters = useCallback(() => {
    setFilters({
      search: "",
      hasImage: "all",
      consoleTypeId: null,
      stationId: null,
    });
  }, [setFilters]);

  const activeFilterCount =
    (filters.search.trim() === "" ? 0 : 1) +
    (filters.hasImage === "all" ? 0 : 1) +
    (filters.consoleTypeId === null ? 0 : 1) +
    (filters.stationId === null ? 0 : 1);

  return { filters, setFilters, clearFilters, activeFilterCount };
}
