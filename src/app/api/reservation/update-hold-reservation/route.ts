  ...
  try {
    ...
    try {
      ...
      if (newConsoleTypeId !== undefined && newConsoleTypeId !== reservation.console_type_id) {
        const toFree: number[] = [reservation.game1_id, reservation.game2_id, reservation.game3_id]
          .filter((x): x is number => x !== null);
        if (toFree.length > 0) {
          await conn.query(
            `SELECT id FROM games WHERE id IN (${toFree.map(() => '?').join(',')}) FOR UPDATE`,
            toFree
          );
          await conn.query(
            `UPDATE games SET holding = 0 WHERE id IN (${toFree.map(() => '?').join(',')})`,
            toFree
          );
        }
        ...
        updates.push(
          "console_id = ?",
          "console_type_id = ?",
          "game1_id = NULL", "game2_id = NULL", "game3_id = NULL",
          "accessoirs = NULL",
          "cours = NULL",
          "date = NULL", "time = NULL"
        );
      ...
      if (!consoleChanged) {
        const desiredGame1 = game1Id === undefined ? reservation.game1_id : game1Id;
        const desiredGame2 = game2Id === undefined ? reservation.game2_id : game2Id;
        const desiredGame3 = game3Id === undefined ? reservation.game3_id : game3Id;

        const desiredListRaw = [desiredGame1, desiredGame2, desiredGame3];
        const desiredList = desiredListRaw.filter((x) => x !== null) as number[];
        const hasDup = new Set(desiredList).size !== desiredList.length;
        if (hasDup) {
          await conn.rollback();
          return NextResponse.json(
            { success: false, message: "Le même jeu ne peut pas être choisi deux fois." },
            { status: 400 }
          );
        }

        const currentSet = new Set<number>(
          [reservation.game1_id, reservation.game2_id, reservation.game3_id]
            .filter((x): x is number => x !== null)
        );
        const finalSet = new Set<number>(desiredList);

        const toAdd: number[] = [...finalSet].filter(id => !currentSet.has(id));
        const toRemove: number[] = [...currentSet].filter(id => !finalSet.has(id));

        const lockIds = [...new Set([...toAdd, ...toRemove])];
        if (lockIds.length > 0) {
          await conn.query(
            `SELECT id FROM games WHERE id IN (${lockIds.map(() => '?').join(',')}) FOR UPDATE`,
            lockIds
          );
        }
        ...
        if (toAdd.length > 0) {
          const [chk] = await conn.query<RowDataPacket[]>(
            `SELECT id FROM games WHERE id IN (${toAdd.map(() => '?').join(',')}) AND holding = 0`,
            toAdd
          );
          ...
          if (blocked.length > 0) {
            await conn.rollback();
            return NextResponse.json(
              { success: false, message: `Jeu(x) indisponible(s): ${blocked.join(", ")}` },
              { status: 400 }
            );
          }
        }
        ...
        if (toRemove.length > 0) {
          await conn.query(
            `UPDATE games SET holding = 0 WHERE id IN (${toRemove.map(() => '?').join(',')})`,
            toRemove
          );
        }
        if (toAdd.length > 0) {
          await conn.query(
            `UPDATE games SET holding = 1 WHERE id IN (${toAdd.map(() => '?').join(',')})`,
            toAdd
          );
        }
        ...
        if (game1Id !== undefined) {
          ...
          else { gameUpdates.push(`game1_id = ?`); gameValues.push(desiredGame1); }
        }
        if (game2Id !== undefined) {
          if (desiredGame2 === null) gameUpdates.push(`game2_id = NULL`);
          else { gameUpdates.push(`game2_id = ?`); gameValues.push(desiredGame2); }
        }
        if (game3Id !== undefined) {
          if (desiredGame3 === null) gameUpdates.push(`game3_id = NULL`);
          else { gameUpdates.push(`game3_id = ?`); gameValues.push(desiredGame3); }
        }
