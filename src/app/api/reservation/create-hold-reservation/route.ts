...
type NombreConsolesRow = RowDataPacket & {
  nombre: number;
};
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      const [rows] = await connection.query<NombreConsolesRow[]>(
        "SELECT nombre FROM consoles WHERE id = ?;",
        [consoleId]
      );
    ...
    return NextResponse.json(
      {
        success: false,
        message: message || "Erreur lors de la création de la réservation",
      },
      { status: 500 }
    );
  }
