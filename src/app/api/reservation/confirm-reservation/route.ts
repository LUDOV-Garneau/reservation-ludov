...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (
      typeof body.reservationHoldId !== "string" ||
      body.reservationHoldId.trim() === ""
    ) {
      return NextResponse.json(
        {
          success: false,
    ...
    if (body.game2Id !== undefined && body.game2Id !== null) {
      ...
      if (!Number.isFinite(game2Id) || game2Id <= 0) {
        return NextResponse.json(
          {
            success: false,
            message: "game2Id must be a positive number if provided",
          },
          { status: 400 }
        );
      }
    ...
    if (body.game3Id !== undefined && body.game3Id !== null) {
      ...
      if (!Number.isFinite(game3Id) || game3Id <= 0) {
        return NextResponse.json(
          {
            success: false,
            message: "game3Id must be a positive number if provided",
          },
          { status: 400 }
        );
      }
      ...
      for (let i = 0; i < body.accessoryIds.length; i++) {
        ...
        if (!Number.isFinite(accId) || accId <= 0) {
          return NextResponse.json(
            {
              success: false,
              message: `accessoryIds[${i}] must be a positive number`,
            },
            { status: 400 }
          );
        }
    ...
    if (hours < 0 || hours > 23 || minutes < 0 || minutes > 59) {
      return NextResponse.json(
        {
          success: false,
          message: "Invalid time value. Hours must be 0-23 and minutes 0-59",
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      if (holdRows.length === 0) {
        ...
        return NextResponse.json(
          {
            success: false,
            message: "Reservation hold not found or does not belong to user",
          },
          { status: 404 }
        );
      }
      ...
      if (hold.user_id !== Number(user.id)) {
        ...
        return NextResponse.json(
          {
            success: false,
            message: "Unauthorized access to this reservation hold",
          },
          { status: 403 }
        );
      }
      ...
      if (
        hold.game1_id !== reservationData.game1Id ||
      ...
      const gameIds = [game1Id, game2Id, game3Id].filter(
      ...
      if (gameIds.length > 0) {
        ...
        if (games.length !== gameIds.length) {
          ...
          return NextResponse.json(
            {
              success: false,
              message: "One or more selected games do not exist",
            },
            { status: 400 }
          );
        }
      ...
      const [result] = await connection.query(
        ...
        [
          ...
          reservationData.accessoryIds
            ? JSON.stringify(reservationData.accessoryIds)
            : null,
          reservationData.coursId,
